...
import { Plus, X, Upload, Calendar, Users } from 'lucide-react'
...
export default function TourForm({ initialData, tourId }: TourFormProps) {
  ...
  const meta = categoryMeta[resolveCategory(form.category)]
    ...
    <form onSubmit={handleSubmit} className="space-y-6">
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 items-start">
        ...
        <div className="space-y-6 lg:sticky lg:top-24">
          <Panel title="Summary">
            <div className="space-y-3">
              <span className={`inline-flex items-center gap-1.5 text-xs font-medium px-3 py-1 rounded-full text-white ${meta.badgeClass}`}>
                {meta.emoji} {meta.label}
              </span>
              <div className="font-display text-lg font-semibold text-white leading-snug line-clamp-2">{form.title || 'Untitled tour'}</div>
              {form.route && <div className="text-xs text-white/50 line-clamp-2">{form.route}</div>}
              <div className="border-t border-white/5 pt-3 space-y-2 text-sm">
                {form.startDate && (
                  <div className="flex items-center gap-2 text-white/60">
                    <Calendar size={13} className="text-gilt-400 shrink-0" />
                    {new Date(form.startDate).toLocaleDateString('en-IN', { day: 'numeric', month: 'short', year: 'numeric' })}
                    {form.endDate && ` – ${new Date(form.endDate).toLocaleDateString('en-IN', { day: 'numeric', month: 'short' })}`}
                  </div>
                )}
                <div className="flex items-center gap-2 text-white/60"><Users size={13} className="text-gilt-400 shrink-0" /> {form.availableSeats} / {form.totalSeats} seats</div>
              </div>
